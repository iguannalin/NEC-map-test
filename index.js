window.addEventListener("load", () => {
  ...
  let filters = {}; // { "Mutual Aid" : { organizations: [ "Upstream Podcast" ], colorClass:
                    // "filter-color-0" }, ... }
  let filtersLeadership = {}; // { "Black-led" : { organizations: [ "Common Future" ], colorClass:
                              // "filter-color-0" }, ... }
  let filtersAreas = {}; // { "Direct Legal Services" : { organizations: [ "New Economy Project" ],
                         // colorClass: "filter-color-0" }, ... }
  let colorIndex = 0;
      ...
      fetchMemberData().then((data) => {
        ...
        data.forEach((entry) => {
          if ( entry["Country"] ) {
            ...
            if ( country === "united states" ) country = "united states of america"; // convert for
                                                                                     // geodb,
                                                                                     // because
                                                                                     // geodb calls
                                                                                     // it the USA
              ...
              if ( !places[state + "+" + country] ) { // if state doesn't already exist in store (places.json)
                ...
                fetch(`http://geodb-free-service.wirefreethought.com/v1/geo/countries/US/regions?namePrefix=${ state }&hateoasMode=false&limit=5&offset=0`).then((d) => d.json()).then((r) => {
                  if ( r && r.data ) {
                    ...
                    if ( place ) {
                      // then use iso code to find the largest admin region of that state, and use
                      // that as the state geo coords
